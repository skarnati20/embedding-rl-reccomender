...
def combine_and_shuffle_csvs(csv_files, output_file):
    ...
    combined_df.to_csv(output_file, index=False, quoting=1)  # quoting=1 is csv.QUOTE_ALL
    print("Done!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Combine and shuffle multiple CSV files with identical columns")
    parser.add_argument('files', nargs='+', help='CSV files to combine')
    parser.add_argument('-o', '--output', default='combined_shuffled.csv', help='Output file name')
