...
class EmbeddingDistribution:
    ...
    def update_reward_average(self, new_reward):
        ...
        self.reward_average += (new_reward - self.reward_average) / self.rewards_received
    ...
    def update_embedding(self, embedding_index, reward):
        ...
        for i in range(len(self.embeddings)):
            ...
            if i == embedding_index:
                new_preference_val = old_preference_val + self.alpha * (reward - self.reward_average) * (1 - self.calculate_action_probability(i))
            else:
                new_preference_val = old_preference_val - self.alpha * (reward - self.reward_average) * self.calculate_action_probability(i)
    ...
    def recommend_embedding(self, temperature=1.0, target_exploration_prob=0.3, embedding_dim=384):
        ...
        if not self.embeddings:
            ...
            return random_embedding.tolist(), -1  # -1 indicates this is a generated vector
        ...
        if np.random.random() < target_exploration_prob:
            ...
            max_z = max(self.preferences)
            exp_values = [math.exp((val - max_z) / temperature) for val in self.preferences]
            sum_exp = sum(exp_values)
